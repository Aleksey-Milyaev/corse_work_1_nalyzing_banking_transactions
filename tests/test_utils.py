...
def transaction():
    ...
    return [
        ...
        {
            "Дата операции": "28.02.2018 22:52:20",
...
@pytest.mark.parametrize(
    "user_date, expected",
    [
        ("2020-10-10 02:00:00", "Доброй ночи"),
...
def test_read_operation_xlsx(transaction):
    ...
    mock_read = Mock(return_value=pd.DataFrame(transaction))
    pd.read_excel = mock_read
    assert read_transactions("2021-01-01 00:00:00") == [
        {
            "Дата операции": "01.01.2021 18:08:23",
            "Дата платежа": "01.01.2021",
            "Номер карты": "*7197",
...
def test_top_transaction(transaction):
    ...
    assert top_transaction(transaction) == [
        {
            "Дата операции": "01.01.2021 18:08:23",
            "Дата платежа": "01.01.2021",
            "Номер карты": "*7197",
...
def test_currency_rate(mock_get):
    ...
    mock_get.return_value.json.return_value = {
        "base": "USD",
        "last_updated": 1646054100,
        "exchange_rates": {"RUB": 104.99999999999999},
    }
...
def test_stocks_price(mock_get):
    ...
    mock_get.assert_called_once_with(f"http://api.marketstack.com/v1/eod?access_key={api_key}&symbols=AAPL")
